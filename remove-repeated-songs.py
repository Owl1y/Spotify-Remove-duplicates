...
current_pl_uris, current_pl_names = get_song_ids(current_playlist[0])


for nums in former_playlist:
    former_pl_uris, former_song_name_pl = get_song_ids(nums)
    # print(former_song_name_pl)
    duplicate_uris = [songs for songs in former_pl_uris if songs in current_pl_uris]
    duplicate_name = [name for name in former_song_name_pl if name in current_pl_names]
